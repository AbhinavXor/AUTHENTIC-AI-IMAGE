...
class SambaNovaProviderAdapter:
    ...
    def is_configured(self) -> bool:
        return bool(
            sambanova_settings.enabled
            and sambanova_settings.api_key
        )

    def _get_client(self) -> httpx.AsyncClient:
        if not self.is_configured():
            raise ProviderError(
                "SambaNova is disabled or its API key is missing.",
...
""".strip()
    ...
    def _translate_error(
        ...
        if isinstance(
            ...
            if status_code == 402:
                return ProviderError(
                    "SambaNova generation requires "
                    "billing or available credits.",
                    provider="sambanova",
                    code="billing",

                    # Central router may continue to
                    # Gemini or Groq.
                    retryable=True,
